...
export default function PrintIndCasualWorksheet({order, onDone}) {
    // ----------------------------
    ...
    const fillData = () => {
        sampleData = {
            ...
            // amountDue: 3000,
            amount: money.format(order.amount ?? 0),
    ...
    function PrintPDF(doc) {
        ...
        const generatePdf = async () => {
            ...
            if (response.headers.get('Content-Type')?.includes('application/pdf')) {
                ...
                a.href = url;
                a.download = 'IndividualCasualHireWorksheet.pdf';
                a.click();
    ...
    const updateTemplateWithData = async () => {
        try {
            const template = await getCMSTemplate({Name: "Individual Casual Hire Worksheet"});
